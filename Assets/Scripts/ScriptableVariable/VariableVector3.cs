...
{
    [CreateAssetMenu(fileName = "VariableVector3", menuName = "Variable/VariableVector3")]

    public class VariableVector3 : ScriptableObject
    {
    ...
    public class VariableVector3PropertyDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            else if (_currentModeIndex == 1) // Édition de la variable
            {
                if (value != null)
                {
                    ...
                    newVector.x = EditorGUI.DelayedFloatField(valueRect, value.vector3Value.x);
                    labelRect.x += coordinateRect.width;
                    valueRect.x += coordinateRect.width;
                    EditorGUI.LabelField(labelRect, "Y");
                    newVector.y = EditorGUI.DelayedFloatField(valueRect, value.vector3Value.y);
                    labelRect.x += coordinateRect.width;
                    valueRect.x += coordinateRect.width;
                    EditorGUI.LabelField(labelRect, "Z");
                    newVector.z = EditorGUI.DelayedFloatField(valueRect, value.vector3Value.z);
                    value.vector3Value = newVector;
                }
                else
                {
